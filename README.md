...
A lightweight .NET library which allow you to decorate handlers (method invoke) with your own custom middlewares.
...
* there are no single interface to invoke handlers
...
* middlewares (behaviors) resolved only once at injection stage, and instances will be reused for each handler invoke
